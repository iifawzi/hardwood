...
public final class IntEqBatchMatcher implements IntBatchMatcher {

    private final int literal;

    public IntEqBatchMatcher(int literal) {
    ...
    public void test(BatchExchange.Batch batch, long[] outWords) {
        ...
        // Build the predicate bitmap ignoring nulls.
        for (int w = 0; w < fullWords; w++) {
            int base = w << 6;
            long word = 0L;
            for (int b = 0; b < 64; b++) {
                word |= ((vals[base + b] == literal) ? 1L : 0L) << b;
            }
            outWords[w] = word;
        }
        if (tail != 0) {
